...
#Apply SHAP-XGB to each environment
for (target in 1:9) {
  
  #Create directory for the target environment
...
SHAP.int.RemoveMain <- as.list(numeric(9))
for(i in 1:9){
...
axis(1, at = 1:9, labels = colnames(Y_Ori)[c(5, 1, 7, 8, 9, 2, 4, 6, 3)], las = 1)
...
Shap_long <- as.list(numeric(9))
names(Shap_long) <- colnames(Y_Ori)
Shap_int <- as.list(numeric(9))
